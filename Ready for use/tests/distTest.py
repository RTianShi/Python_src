...
def applyMR_Assert(originalInput1_1, originalInput1_2, originalResult):
    ...
    # OR9:
    check.is_true(originalResult <= transformResult9, "MR9 failed")
...
@pytest.mark.parametrize("input1,input2", [
    ([15.2, 15.6, 13.4], [10.1, 15.6, 13.4]),
    ([5.99, 4.83, 7.26, 9.45], [6.72, 4.61, 7.23, 8.12]),
    ([3.14, -2.718], [3.00, -2.70]),
    ([-7.89, 1.234, 0.567], [-7.90, 1.230, 0.570]),
    ([4.56, 6.13, 7.88], [8.96, 9.50, 4.60]),
    ([4.5678, -3.4567, 2.3456], [4.567, -3.456, 2.345]),
    ([-1.234, 2.345, -3.456, 4.567], [-1.23, 2.35, -3.46, 4.56]),
    ([5.12, 7.56, 8.19], [5.12, 6.79, 0.46]),
    ([-3.14159, 2.71828], [-3.14, 2.72]),
    ([1.23, 2.24, 3.25, 4.26], [5.27, 6.28, 9.29, 10.30]),
])
def test_dist_with_func(input1, input2):
    func = runner.CURRENT_MUTANT_FUNC
    assert func is not None, "mutant_func 没有被注入"
