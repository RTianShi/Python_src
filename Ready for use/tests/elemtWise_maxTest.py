# mutants/tests/test_elemtWise_max_py2_style.py
import copy
...
print("test_elemtWise_max_py2_style 执行中")
...
def applyMR_Assert(originalInput1: List[int], originalInput2: List[int], originalResult: List[int]):
    ...
    # MR9: 复合转换一致性 (这里保留，但 Java 中未启用断言)
    ...
    # MR15: 类三角函数周期性 -> Java 中未启用断言（仍计算以备后用）
...
def test_elemtWise_max_with_func(a, b):
    func = runner.CURRENT_MUTANT_FUNC
